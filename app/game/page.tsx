...
import { GameProvider, useGame } from '../../contexts/GameContext'; // Import du GameProvider
import Button from '../../components/ui/Button';
import HUD from '../../components/game/HUD'; // Composant que nous allons créer
import GameView from '../../components/game/GameView'; // Composant que nous allons créer
import router from 'next/router';

function GameContent() {
  const { room, loading, error, isHost } = useRoom();
  const { isPaused } = useGame(); // On récupère l'état de pause depuis le GameContext
  const router = useRouter();

  useEffect(() => {
    if (loading) return;
    // Si la room n'existe pas ou que son statut n'est pas 'running', on redirige
  ...
  // Logique de gestion du nombre de joueurs
  const playersConnected = Object.values(room.players).filter(
    (p) => p.connected
  ).length;
    ...
    <div className="game-page">
      ...
      {!isPlayerCountCorrect && (
        <div className="player-count-overlay">
          ...
          <p>
            Session verrouillée en attente du nombre de joueurs requis (
      ...
      {/* Overlay si la partie est terminée */}
      {room.status === 'ended' && (
